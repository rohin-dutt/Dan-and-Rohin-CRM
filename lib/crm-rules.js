...
export function getRelationshipStatus(person, todayValue = new Date(), followUpDate = null) {
  ...
  // Someone contacted in the last 7 days is never overdue regardless of cadence
  if (daysSince !== null && daysSince <= 7) return "recent";

  // An open follow-up date takes priority over the cadence calculation
  if (followUpDate) {
    const followUpDay = toDay(followUpDate);
    const today = toDay(todayValue);
    if (followUpDay && today) {
      const followUpDays = Math.ceil((followUpDay.getTime() - today.getTime()) / MS_PER_DAY);
      if (followUpDays >= 0 && followUpDays <= 7) return "due_this_week";
      if (followUpDays > 7) return "coming_up";
      // followUpDays < 0 means that follow-up is itself overdue — fall through to cadence
    }
  }
