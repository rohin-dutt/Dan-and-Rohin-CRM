...
set
  birthday_month = coalesce(birthday_month, extract(month from birthday)::integer),
  birthday_day = coalesce(birthday_day, extract(day from birthday)::integer),
  birthday_year = coalesce(birthday_year, extract(year from birthday)::integer)
where birthday is not null;

alter table public.people
  add constraint people_birthday_parts_complete_check
    check (
      (birthday_month is null and birthday_day is null and birthday_year is null)
      or (birthday_month is not null and birthday_day is not null)
    ),
  add constraint people_birthday_month_check
    check (birthday_month is null or birthday_month between 1 and 12),
  add constraint people_birthday_day_check
    check (birthday_day is null or birthday_day between 1 and 31),
  add constraint people_birthday_year_check
    check (birthday_year is null or birthday_year between 1 and 9999),
  add constraint people_birthday_valid_date_check
    check (
      birthday_month is null
      or make_date(coalesce(birthday_year, 2000), birthday_month, birthday_day) is not null
    );

create index if not exists people_user_id_birthday_parts_idx
  on public.people (user_id, birthday_month, birthday_day)
...
begin
  ...
  insert into crm_restore_people (
    ...
    birthday_month, birthday_day, birthday_year, birthday, how_met,
    relationship_type, relationship_strength, preferred_contact_method,
    contact_frequency_days, last_contacted_at, notes, created_at
  )
  select
    id,
    name,
    ...
    latitude,
    longitude,
    coalesce(birthday_month, extract(month from birthday)::integer),
    coalesce(birthday_day, extract(day from birthday)::integer),
    coalesce(birthday_year, extract(year from birthday)::integer),
    birthday,
    how_met,
    relationship_type,
  ...
  insert into crm_restore_interactions (
    id, person_id, type, date, notes, follow_up_needed, follow_up_date,
    follow_up_status, follow_up_snoozed_until, created_at
  )
  select
    id,
    person_id,
  ...
  on conflict (id) do update
    set user_id = excluded.user_id, name = excluded.name, color = excluded.color
    where public.tags.user_id = current_user_id;

  insert into public.people (
    id, user_id, name, email, phone, company, role, location, latitude,
    longitude, birthday_month, birthday_day, birthday_year, birthday, how_met,
    relationship_type, relationship_strength, preferred_contact_method,
    contact_frequency_days, last_contacted_at, notes, created_at
  )
  select
    ...
    birthday_day,
    birthday_year,
    case when birthday_year is not null then make_date(birthday_year, birthday_month, birthday_day) else null end,
    how_met,
    relationship_type,
    relationship_strength,
    preferred_contact_method,
