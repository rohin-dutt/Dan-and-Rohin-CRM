...
-- The new parameter list is a different signature from the existing
-- function (an extra trailing default arg), so drop the old one first
-- to avoid ending up with two ambiguous overloads.
drop function if exists public.create_interaction_and_touch_person(uuid, text, date, text, boolean, date, text);
...
grant execute on function public.create_interaction_and_touch_person(uuid, text, date, text, boolean, date, text, text) to authenticated;
